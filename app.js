...
socket.on('newnottification', async (data)=>{
  ...
  let sender = user;
    socket.broadcast.emit('addGroupnottification',{datas:data,sender});
...
socket.on('existschat',async (data)=>{
 ...
 const user = await User.findById(data.receiver_id); 
  ...
  socket.emit('loadchats',{chats:chats,sender_id:data.sender_id,receiver_id:data.receiver_id,user});
...
socket.on('load-deleted-chat',(data)=>{
  socket.broadcast.emit('loading-deleted-chat',data)
})
