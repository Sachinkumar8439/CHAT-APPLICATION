...
current_in = "";
document.current_in = current_in;
// var messages=[];
...
buttons.forEach((button) => {
    // console.log("buttons  pageki",buttons,"button",button)
...
$(document).on("click", ".users", async function () {
  ...
  // $("#chat-window").addClass("translatex");
...
console.log("up to date here poit 3")
...
//     let html =
//       `<li class="users" data-name=" "data-id="` +response_id +`">
...
$(document).on("click", ".message-menu-btn", function () {
  ...
  const message_id = $(this).attr("message-rightId");
  console.log("message id ", message_id);
  let messagepara = document.getElementById(`${message_id}`);
  console.log("message id and message are ", message_id, messagepara);
  ...
  console.log("here up to date point 3");

  document.getElementById("delete-message").addEventListener("click", (e) => {
    e.preventDefault();
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            ...
            $(`#${message_id}-maindiv`).remove();
            console.log("removed succesfully");
            socket.emit("load-deleted-chat", data);
          } else {
            alert("something is wrong from deleteing chat");
          }
        },
      });
    });
...
socket.on("loading-deleted-chat", (data) => {
  if (data) {
    $(`#${data.message_id}-maindiv`).remove();
    console.log("deleted here succsefully");
  } else {
    console.log("user delated chat error and data not coming..");
  }
});
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        let html =
          `
                <div id="${messageId}-maindiv"  class="message right">
                <div style="display: flex;" class="flexing">
                <img message-rightId="` +
          messageId +
          `" id="message-menu-btn" class="message-menu-btn" src="menu-dots-vertical.png" alt="">
                <p id="${messageId}">` +
          chat +
          `</p>
                </div>
...
socket.on("loadnewchat", (data) => {
  if (sender_id == data.receiver_id && receiver_id == data.sender_id) {
    ...
    let html =
      `<div id="${data._id}-maindiv"  class="message left">
        <div message-leftId="` +
      data._id +
      `"  style="display: flex;" class="flexing">
        <p>` +
      data.message +
      `</p>
        </div>
    ...
    if (sender_id == data.receiver_id && receiver_id != data.sender_id) {
      ...
      document.getElementById(
        `${data.sender_id}-unread`
      ).innerText = `${unreadmessages}`;
      document.getElementById(`${data.sender_id}-unread`).style.display =
        "flex";
    }
  }
});
...
socket.on("loadchats", (data) => {
  ...
  $(".messages").html("");
  let html2 = `<div style="display:flex;margin:20px;flex-direction:column;justify-content: center;align-items: center;" class="messageprofile">
                    <img src="man.png" alt="" width="80px">
                    <h3 style="font-size:20px">${data.user.username}</h3>
                    <p style="color:grey">${data.user.name}</p>
                </div>`;
  ...
  for (let i = 0; i < chats.length; i++) {
    ...
    if (chats[i]["sender_id"] == sender_id) {
      ...
      img =
        `<img message-rightId="` +
        chats[i]._id +
        `" class="message-menu-btn" src="menu-dots-vertical.png" alt="">`;
    } else {
    ...
    html =
      html +
      `
        <div id="${chats[i]._id}-maindiv" div-id="` +
      chats[i]._id +
      `" class="` +
      addclass +
      `">
...
socket.on("loadtyping", (data) => {
  if (data) {
    if (sender_id === data.id && receiver_id === data.sender_id) {
      ...
      if (!istyping) {
        let html = `
                <div id="typing" style="display:flex;"  class="message left">
                <img style="width:20px" src="man.png">
                <div style="display: flex;" class="flexing">
                <p style="font-size:12px;margin-left:10px">typing...</p>
                </div>
                </div>`;
        $(".messages").append(html);
        istyping = true;
...
if (unread != 0) {
  $("#nottification-btn").append(
    `<span id="notification-badge" style="font-size: 12px;border-radius: 5px;background-color: black;color: white;padding: 3px;position: absolute;margin-top: -10px;">${unread}</span>`
  );
